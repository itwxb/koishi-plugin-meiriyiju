...
export const Config: Schema<Config> = Schema.object({
  ...
  apiUrl: Schema.string().default('https://v1.hitokoto.cn').description('一言 API 地址'),
  timeout: Schema.number().default(5000).min(1000).max(30000).description('请求超时时间（毫秒）'),
  keywords: Schema.string().default('每日一句').description('触发关键词（多个用逗号分隔）'),
})
 ...
 */
async function fetchHitokoto(ctx: Context, url: string, timeout: number): Promise<string> {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.middleware(async (session, next) => {
    ...
    try {
      ...
      const quoteElement = replyQuote ? h('quote', { id: session.messageId }) : ''
